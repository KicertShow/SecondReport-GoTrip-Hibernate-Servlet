		...
		<c:if test="${ViewBean != null}">
			<form action="Edit_view_pro" method="post" width="50%">
				<input type="hidden" name="prod_no"
					value="<c:out value='${ViewBean.prod_no}' />" />
			</form>
		</c:if>
		<c:if test="${ViewBean == null}">
					...
					<div class="card">
						<div class="card-header">
							...
							<table class="table table-hover table-sm tableStyle table-bordered">
								...
								<tbody>
									<c:forEach var="ViewBean" items="${list}">
										<tr>
											<td><c:out value="${ViewBean.prod_no}" /></td>
											<td><c:out value="${ViewBean.prod_name}" /></td>
											<td class="ellipsis">
												<textarea readonly rows="5" cols="30" class="textarea" style="white-space: normal">
													<c:out value="${ViewBean.prod_Intro}" />
												</textarea>
											</td>
											<td><c:out value="${ViewBean.open_week}" escapeXml="false" /></td>
											<td><c:out value="${ViewBean.open_time}" /></td>
											<td><c:out value="${ViewBean.end_time}" /></td>
											<td><c:out value="${ViewBean.phone}" /></td>
											<td><c:out value="${ViewBean.country}" /></td>
											<td><c:out value="${ViewBean.city}" /></td>
											<td><c:out value="${ViewBean.location}" /></td>
											<td class="ellipsis"><c:out value="${ViewBean.address}" /></td>
											<td><c:out value="${ViewBean.price}" /></td>
											<%-- <td><a href="Delete_view_pro?id=<c:out value='${view_pro.prod_no}'class="btn btn-outline-warning btn-sm" />">Delete</a></td> --%>
											<td><div style="float: left">
													<a href="Edit_view_pro?prod_no=<c:out value='${ViewBean.prod_no}'/>">
														<i class="bi bi-pencil-square"></i>編輯
													</a>
													<br> 
													<a href="Delete_view_pro?prod_no=<c:out value='${ViewBean.prod_no}'/>">
