...
<body>
	...
	<main id="right-panel" class="right-panel">
		...
		<c:if test="${ViewBean != null}">
			...
			<input type="hidden" name="prod_no" value="<c:out value='${ViewBean.prod_no}' />" />
		</c:if>
							...
							<table class="table table-hover table table-sm">
								...
								<!-- 商品名稱 -->
								 <div class="mb-4 row">
                                    ...
                                    <div class="col-sm-9">
                                    <input type="text" name="prod_name" class="form-control" required="required"  value="<c:out value='${ViewBean.prod_name}' />" />
<%--                                     <input type="text" name="prod_name" class="col-sm-3 col-form-label" value="<c:out value='${view_pro.prod_name}' />" /> --%>
								...
								<div class="mb-4 row">
                                    ...
                                    <div class="col-sm-9">
									<textarea name="prod_Intro" class="form-control" cols="30" rows="10" style="white-space: normal"><c:out value='${ViewBean.prod_Intro}' /></textarea>
<%-- 								<input type="text" name="prod_Intro" "<c:out value='${view_pro.prod_Intro}' />" /> --%>
								...
								<div class="mb-4 row">
                                    ...
                                    <div class="col-sm-9">
											<input type="checkbox" name="week1" 
												<c:if test="${fn:containsIgnoreCase(ViewBean.open_week, '1')}"> checked</c:if>			
											 />
											 <span>星期一</span>
											<input type="checkbox" name="week2"
												<c:if test="${fn:containsIgnoreCase(ViewBean.open_week, '2')}"> checked</c:if>	
											/>
											 <span>星期二</span>
											<input type="checkbox" name="week3"
												<c:if test="${fn:containsIgnoreCase(ViewBean.open_week, '3')}"> checked</c:if>	
											/>
											<span>星期三</span>
											<input type="checkbox" name="week4"
												<c:if test="${fn:containsIgnoreCase(ViewBean.open_week, '4')}"> checked</c:if>	
											/>
											<span>星期四</span>
											<input type="checkbox" name="week5"
												<c:if test="${fn:containsIgnoreCase(ViewBean.open_week, '5')}"> checked</c:if>	
											/>
											<span>星期五</span>
											<input type="checkbox" name="week6"
												<c:if test="${fn:containsIgnoreCase(ViewBean.open_week, '6')}"> checked</c:if>	
											/>
											<span>星期六</span>
											<input type="checkbox" name="week7"
												<c:if test="${fn:containsIgnoreCase(ViewBean.open_week, '7')}"> checked</c:if>	
											/>
											<span>星期日</span>
									</div>
								</div>
                                    ...
                                    <label class="col-sm-3 col-form-label" >營業起始時間</label>
                                    	<div class="col-sm-9">
											<input type="time" name="open_time" value="<c:out value='${ViewBean.open_time}' />">
										</div>
									</div>
									<!--營業結束時間 -->
									<div class="mb-4 row">
									<label class="col-sm-3 col-form-label" >營業結束時間</label>
										<div class="col-sm-9">
											<input type="time" name="end_time" value="<c:out value='${ViewBean.end_time}'/>" >
										</div>
									</div>
								<!-- 電話 -->
								 <div class="mb-4 row">
                                    ...
                                    <div class="col-sm-9">
	                                    <input type='text' name="phone" required="required" maxlength="10" data-minlength="10" placeholder="請輸入電話號碼"  value="<c:out value='${ViewBean.phone}' />">
										<!--  pattern="09\d{2}\d{6}"/ -->
                                    </div>
                                </div>
								<!-- 國家 -->
								...
								<div class="mb-4 row">
                                    ...
                                    <div class="col-sm-9">
                                    	<input type="text" name="address" class="form-control" value="<c:out value='${ViewBean.address}' />" />
									</div>
								</div>
								...
								<div class="mb-4 row">
                                    ...
                                    <div class="col-sm-9">
                                    	<input type="text" name="price" class="form-control"  value="<c:out value='${ViewBean.price}' />" />
									</div>
								</div>
	...
	//判斷城市資料後再自動代入地區
		window	.addEventListener('load', function () {
			...
			setTimeout(function(){
				...
				document.getElementById("city").value = "<c:out value='${ViewBean.city}'/>";
				document.getElementById("city").dispatchEvent(new Event('change'));
				setTimeout(function(){
					document.getElementById("dist").value = "<c:out value='${ViewBean.location}'/>";
				},1000);
			},1000);
		})
	</script>
